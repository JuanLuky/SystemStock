...
@Component({
  ...
  imports: [
    ...
    MatSortModule,
    MatPaginatorModule,
    DatePipe,MatFormFieldModule, MatInputModule, MatDatepickerModule, CommonModule, SidebarComponent
  ],
...
export class ReleaselistComponent  {
  ...
  displayedColumns: string[] = ['tipo', 'quantidade', 'data', 'produto'];
  ...
  constructor(private _httpClient: HttpClient, private _service: StockHttpService) {
   ...
   refresh(pageEvent: PageEvent = {length: 0, pageIndex: 0, pageSize: 10 }) {
    ...
    this.data$ = this._service.listarMovimentacoes(pageEvent.pageIndex, pageEvent.pageSize)
    .pipe(
      tap(() => {
